...
支持 4 种内置主题：浅色、Nord、蓝灰（类 VS Code）、暖灰（类 Typora），主题入口已集中到配置层，后期可通过插件文件动态注入新主题。
...
- 编辑器能力已拆分为独立模块，便于扩展和维护
- 工具栏采用控制层、视图层、配置层分离的结构
- 工作区状态由独立 hook 管理，减少页面组件耦合
...
结构优化说明请参阅 [项目结构优化说明](./docs/项目结构优化说明.md)
